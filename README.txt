...
1. As this repo was changed from original fork to build 'ranger-<version>-trino-plugin.tar.gz' which depends on JDK11. You might encounter error in the build process for 'hive-agent' module. Ranger uses hive version 3.1.2 which still requires JDK8 while building so to avoid this error please use this instruction. You can use original build process on JDK8 to seperately build the 'ranger-<version>hive-plugin.tar.gz'.

2. Use ranger-2.1.0-trino branch for Ranger-2.1.0 version
